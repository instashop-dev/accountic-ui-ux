# Spec: admin-analytics-gate

## Overview
The admin navigation conditionally renders the Analytics link based on the `ANALYTICS_ENABLED` environment variable. When the flag is absent or not `'true'`, the link is omitted entirely and the analytics route returns 404.
